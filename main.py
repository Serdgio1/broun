...
horizontal = Scale(tk, length=W / 2 * 1.5, from_=0, to=100, orient=HORIZONTAL, command=lambda val: print(value.get()),
                   variable=value)
horizontal.pack()


class Ball:
    def __init__(self, size, color, speedx, speedy, x, y):
        # x = randint(0,W-size)
        # y = randint(0,H-size)
        self.x = x
        self.y = y

        if x and x - size and x + size not in lis:
    ...
    def movement(self):
        ...
        if value.get() >= 50:
            ...
            self.create[3] = True
            if self.create[4]:
                square = canvas.create_rectangle(pos[0] - 200, pos[1] - 200, pos[2] + 200, pos[3] + 200, outline='',
                                                 tags='square4')
            ...
            self.create[2] = True
            if self.create[3]:
                square = canvas.create_rectangle(pos[0] - 90, pos[1] - 90, pos[2] + 90, pos[3] + 90, outline='',
                                                 tags='square3')
                self.pos_s = canvas.coords(square)
                self.create[3] = False
            self.not_move(self.pos_s, pos)
            ...
            if self.create[2]:
                ...
                square = canvas.create_rectangle(pos[0] - 50, pos[1] - 50, pos[2] + 50, pos[3] + 50, outline='',
                                                 tags='square2')
                self.pos_s = canvas.coords(square)
                self.create[2] = False
            self.not_move(self.pos_s, pos)
            ...
            if self.create[1]:
                canvas.config(borderwidth=0)
                square = canvas.create_rectangle(pos[0] - 20, pos[1] - 20, pos[2] + 20, pos[3] + 20, outline='',
                                                 tags='square1')
                self.pos_s = canvas.coords(square)
                self.create[1] = False
            self.not_move(self.pos_s, pos)
            ...
            if self.create[0]:
                canvas.config(borderwidth=0)
                square = canvas.create_rectangle(pos[0] - 1, pos[1] - 1, pos[2] + 1, pos[3] + 1, outline='',
                                                 tags='square0')
                self.pos_s = canvas.coords(square)
                self.create[0] = False
            self.not_move(self.pos_s, pos)
...
for i in range(38):
    ...
    for i in range();
    for obj in objs:
        if obj.x - 40 < x and obj.x + 40 > x and obj.y - 40 < y and obj.y + 40 > y:
            objs.append(Ball(40, 'yellow', 0, 0, x, y))
        else:
            cringe_count += 1
tk.mainloop()
